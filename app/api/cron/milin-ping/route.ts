...
import { getMilinMemory, searchVault } from "@/lib/vault";
...
async function buildPingPrompt(type: MessageType): Promise<string> {
...
เขียน LINE message ที่ Milin จะส่งหา Max โดยไม่รอให้เขา text ก่อน
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const prompt = await buildPingPrompt(type);
